...
let Ability = (ability: IAbility) => {
  ...
  let getAbilitInfo = (a: AbilityType) => {
    ...
    if (a != undefined) {
      ...
      if (abilities1 != undefined) {
        let effectEntries: AbilityDetails =
          abilities1 as unknown as AbilityDetails;
        let items = (
          <React.Fragment>
            <div id={a.name}>
              <div>{a.name}:</div>
              <div className="hbox">{effectEntries.effect}</div>
            </div>
          </React.Fragment>
        );
        col.push(items);
  ...
  const getAbility = () => {
    ...
    if (ability) {
      able = <div key={"key"}>{getAbilitInfo(abilities as AbilityType)}</div>;
    }
    return able;
  };
