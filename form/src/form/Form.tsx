...
let Form = (index: any) => {
  ...
  useEffect(() => {
    fetchForm().then((r) => console.log("got getting form"));
  }, [index.index]);
  let fetchForm = async () => {
    console.log("setting form");
    let data = await fetch(
      "https://pokeapi.co/api/v2/pokemon-form/" + index.index
    );
    let f = await data.json();
    setForm(f);
  };

  function getType(type1: {
    slot: number;
  ...
  function getSrc() {
    // @ts-ignore
    let src: string = form["sprites"]
      ? (form["sprites"]["front_default"] as string)
      : "";
